...
def load_and_train():
    """Lightweight loader used by the Streamlit app.

    This function only loads precomputed artifacts from disk. Training
    has been moved to `train_and_save.py` to avoid heavy work at
    Streamlit startup (prevents long cold-start times).
    """
    if ARTIFACT_PATH.exists():
        return joblib.load(ARTIFACT_PATH)

    # If no artifact present, instruct user to run training locally.
    st.warning("Model artefak belum ditemukan. Untuk mempercepat startup, jalankan training lokal sekali.")
    st.markdown("**Cara membuat artefak (di mesin lokal):**")
    st.code("python train_and_save.py", language="bash")
    st.info("Setelah artefak dibuat, refresh halaman ini. Untuk saat ini app hanya menunggu artefak.")
    st.stop()
